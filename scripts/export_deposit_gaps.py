#!/usr/bin/env python3
"""Export orders with deposit 2100 gaps (non-zero net balance) to CSV.

Uses correct payment_transaction → order linking (pt.order_id, not je.source_id).
"""
import sqlite3, csv
from pathlib import Path

DB_PATH = Path(__file__).resolve().parent.parent / "data" / "baker.db"
OUT_PATH = Path(__file__).resolve().parent.parent / "deposit_revenue_gap.csv"
...
def main():
    ...
    conn.close()
    print(f"Saved {len(rows)} rows ({issue_count} with issues) to {OUT_PATH}")
