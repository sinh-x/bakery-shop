...
def test_v58_migrates_customers_created_by_v57(use_memory_db):
    """Customers generated by v57 must have their phones migrated by v58.

    Exercise the historical v57 -> v58 data-migration sequence directly.
    Rewinding ``schema_version`` on a current schema is not a valid legacy
    database simulation because it re-runs later one-shot DDL such as v67's
    ``acknowledged_at`` column addition.
    """
    from baker.db.connection import get_db

    with get_db() as conn:
        ensure_schema(conn)
        # Reset the data owned by these migrations and remove v58's table so
        # v57 runs against the same schema boundary it encountered originally.
        conn.execute("UPDATE orders SET customer_id = NULL")
        conn.execute("DROP TABLE customer_phones")
