...
const _categories = <Category>[
  Category(id: 1, slug: 'banh_kem', name: 'Bánh kem', codePrefix: 'BK', active: 1),
];
...
class _SwapInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (path == '/api/orders/ORD-SWAP/items/10' && method == 'PATCH') {
      ...
      _patchBody = body;
      // Echo back the updated work item: new productId/name but every other
      // field preserved (mirrors the backend exclude_unset behavior).
      handler.resolve(
        Response(
...
Future<ProviderContainer> _buildContainer(Interceptor interceptor) async {
  ...
  final container = ProviderContainer(overrides: [
    ...
    productsProvider.overrideWith(() => _FakeProductsNotifier(<Product>[_oldProduct, _newProduct])),
    categoriesProvider.overrideWith(() => _FakeCategoriesNotifier(_categories)),
    productPhotoRefreshTickProvider.overrideWith(_FakePhotoRefreshTickNotifier.new),
  ]);
  ...
  group('DG-414 Phase 4.4: WorkItemEditCard "Đổi sản phẩm" swap flow', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(tooltip, OrdersLabels.changeProduct,
            reason: 'FR7: swap button tooltip should use the centralized VN label');
      },
    );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(_patchBody!.keys.toSet(), <String>{'productId', 'productName'},
            reason: 'AC1/AC5: swap PATCH must send only productId + productName; '
                'all other fields must be omitted so they are preserved');
        expect(_patchBody!['productId'], 'P-NEW',
            reason: 'AC1: productId should update to the new product code');
        expect(_patchBody!['productName'], 'Bánh mới',
        ...
        for (final preservedKey in <String>[
          ...
          expect(_patchBody!.containsKey(preservedKey), isFalse,
              reason: 'AC1/AC5: $preservedKey must NOT be sent on swap so the '
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(_patchBody!.containsKey('unitPrice'), isFalse,
            reason: 'AC2: unitPrice must not be sent on swap so it is preserved');
        expect(_patchBody!.containsKey('assignedPrice'), isFalse,
            reason: 'AC2: assignedPrice must not be sent on swap so it is preserved');
      },
    );
