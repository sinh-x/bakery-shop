...
void main() {
  const draftContext = FormDraftContext(
    formType: 'auth-boundary-test',
  ...
  group('AuthNotifier', () {
    ...
    test(
      'login() persists the token and transitions to authenticated',
      () async {
        final dio = Dio()
          ..interceptors.add(
            _LoginOkInterceptor(
        ...
        container
            ...
            .retainDraft(draftContext, 'previous session');
        final draftEpoch = container.read(formDraftSessionEpochProvider);

        await container
            .read(authProvider.notifier)
            .login(username: 'An', password: 'secret');
        ...
        expect(container.read(formDraftSessionEpochProvider), draftEpoch + 1);
      },
    );

    test(
      'logout() clears the token and transitions to unauthenticated',
      () async {
        ...
        container
            .read(formDraftSessionProvider.notifier)
            .retainDraft(draftContext, 'draft');
        final draftEpoch = container.read(formDraftSessionEpochProvider);
    ...
    test('handle401() advances draft epoch when registry is empty', () async {
      final container = ProviderContainer(
        overrides: [sharedPreferencesProvider.overrideWithValue(prefs)],
      );
      addTearDown(container.dispose);
      ...
      await container.read(authProvider.notifier).handle401();
    ...
    test('login() surfaces forcePasswordChange flag when set (FR8)', () async {
      ...
      final dio = Dio()
        ..interceptors.add(
          _LoginOkInterceptor(token: token, forcePasswordChange: true),
        );
      final container = ProviderContainer(
        overrides: [
          sharedPreferencesProvider.overrideWithValue(prefs),
    ...
    test(
      'changePassword() clears session on success (DG-319 Phase 4)',
      () async {
        // Seed authenticated state.
        final token = buildJwt({
          'sub': 'Sinh',
    ...
    test(
      'changePassword() surfaces error on 401 (old password wrong)',
      () async {
        final token = buildJwt({
          'sub': 'Sinh',
          'role': 'admin',
        ...
        container
            .read(formDraftSessionProvider.notifier)
            .retainDraft(draftContext, 'retryable draft');
        final draftEpoch = container.read(formDraftSessionEpochProvider);
        ...
        expect(
          container
              .read(formDraftSessionProvider.notifier)
              .readDraft<String>(draftContext),
          'retryable draft',
        );
        expect(container.read(formDraftSessionEpochProvider), draftEpoch);
      },
    );
    ...
    test('clearForcePasswordChange() clears the flag (FR10)', () async {
      ...
      await container
          .read(authProvider.notifier)
          .login(username: 'Sinh', password: 'secret');
    ...
    test('CQ-11: forcePasswordChange is persisted to storage and restored on '
        'restart (build())', () {
    ...
    test('CQ-11: clearForcePasswordChange() clears the flag from storage so a '
